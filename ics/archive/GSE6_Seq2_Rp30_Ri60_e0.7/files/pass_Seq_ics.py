...
sn_Seq = arepo.Snapshot('Seq_ICs.dat')

npart = np.array( [0,  0,  0,  0,  0,  0])
masses = [0., 0., 0., 0., 0., 0.]
for i in range(6):
    npart[i]  = sn_Seq.NumPart_Total[i] #+ sn_GSE.NumPart_Total[i]
    masses[i] = sn_Seq.MassTable[i]
masses[0] = 0.0

ics = arepo.ICs('Seq.hdf5', npart, masses=masses)
...
ics.part0.pos[:] = sn_Seq.part0.pos + shift
ics.part0.vel[:] = sn_Seq.part0.vel
#ics.part0.mass[:] = sn_GSE.part0.mass    
ics.part0.mass[:] = np.full(sn_Seq.NumPart_Total[0], sn_Seq.MassTable[0])
ics.part0.u[:] = sn_Seq.part0.u

ics.part1.pos[:] = sn_Seq.part1.pos + shift
ics.part1.vel[:] = sn_Seq.part1.vel

ics.part2.pos[:] = sn_Seq.part2.pos + shift
ics.part2.vel[:] = sn_Seq.part2.vel

#ics.part3.pos[:] = sn_GSE.part3.pos + shift
#ics.part3.vel[:] = sn_GSE.part3.vel

# Set metallicities
metal_fractions_Seq_disk = get_initial_mass_fractions(zsolar_Seq_disk) 

metal_frac_arr_Seq_disk = np.full((sn_Seq.NumPart_Total[0], 10), metal_fractions_Seq_disk)

#metal_fractions = np.concatenate((metal_frac_arr_GSE_disk, metal_frac_arr_GSE_halo))
metal_fractions = metal_frac_arr_Seq_disk

ics.part0.GFM_Metals[:] = metal_fractions 

metallicity_Seq_disk = np.full(sn_Seq.NumPart_Total[0], zsolar_Seq_disk * GFM_SOLAR_METALLICITY)
#metallicity = np.concatenate((metallicity_GSE_disk, metallicity_GSE_halo))
metallicity = metallicity_Seq_disk
