...
sn_MW = arepo.Snapshot('MW_ICs.dat-with-grid.hdf5')
...
for i in range(6):
    ...
    npart[i] = sn_MW.NumPart_Total[i]
    masses[i] = sn_MW.MassTable[i]

Nstar = int(star_frac * sn_MW.NumPart_Total[2])
npart[2] -= Nstar
npart[4] = Nstar
...
ics.addField("GFM_StellarFormationTime", pres=[0,0,0,0,1,0])
ics.addField("GFM_InitialMass", pres=[0,0,0,0,1,0])
ics.addField("GFM_Metallicity", pres=[0,0,0,0,1,0])
ics.addField("GFM_Metals", pres=[0,0,0,0,10,0])
...
ics.part2.pos[:] = sn_MW.part2.pos[Nstar:]
ics.part2.vel[:] = sn_MW.part2.vel[Nstar:]
...
ics.part4.pos[:] = sn_MW.part2.pos[:Nstar]
ics.part4.mass[:] = np.full(Nstar, masses[2])
ics.part4.vel[:] = sn_MW.part2.vel[:Nstar]

ics.part4.GFM_StellarFormationTime[:] = np.full(Nstar, 1e-15)
ics.part4.GFM_InitialMass[:] = np.full(Nstar, masses[2])
ics.part4.GFM_Metallicity[:] = np.full(Nstar, zsolar_MW*GFM_SOLAR_METALLICITY)

metal_fractions_MW = get_initial_mass_fractions(zsolar_MW)
ics.part4.GFM_Metals[:] = np.full((Nstar, len(metal_fractions_MW)), metal_fractions_MW)
