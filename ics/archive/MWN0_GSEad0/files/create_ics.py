...
sn_MW = arepo.Snapshot('MW_ICs.dat')
...
GSEmass = np.full(sn_GSE.NumPart_Total[0], sn_GSE.MassTable[0])
MWmass = np.full(sn_MW.NumPart_Total[0], sn_MW.MassTable[0])
ics.part0.mass[:] = np.concatenate((MWmass, GSEmass))
...
MWmass = np.full(sn_MW.NumPart_Total[1], sn_MW.MassTable[1])
GSEmass = np.full(sn_GSE.NumPart_Total[1], sn_GSE.MassTable[1])
ics.part1.mass[:] = np.concatenate((MWmass, GSEmass))
...
MWmass = np.full(sn_MW.NumPart_Total[2], sn_MW.MassTable[2])
GSEmass = np.full(sn_GSE.NumPart_Total[2], sn_GSE.MassTable[2])
ics.part2.mass[:] = np.concatenate((MWmass, GSEmass))

ics.part3.pos[:] = sn_MW.part3.pos
ics.part3.vel[:] = sn_MW.part3.vel
#ics.part3.id[:]  = sn.part3.id
ics.part3.mass[:] = np.full(sn_MW.NumPart_Total[3], sn_MW.MassTable[3])
