...
ics = arepo.ICs('GSE.hdf5', npart, masses=masses)
...
metallicity_GSE_disk = np.full(N_GAS, zsolar_GSE_disk * GFM_SOLAR_METALLICITY)
metallicity_GSE_halo = np.full(sn_GSE.NumPart_Total[0] - N_GAS, zsolar_GSE_halo * GFM_SOLAR_METALLICITY)
...
passive_disk = np.repeat(np.array([0, 0, 1, 0], dtype=float).reshape(1, -1), N_GAS, axis=0)
passive_halo = np.repeat(np.array([0, 0, 0, 1], dtype=float).reshape(1, -1), sn_GSE.NumPart_Total[0] - N_GAS, axis=0)
