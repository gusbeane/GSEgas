...
GFM                                    #master switch
GFM_STELLAR_EVOLUTION=2                #stellar evolution: 0->default, 1->no mass loss (beta value changes + MassMetallicity & MassMetals inconsistent internally with cell dynamical mass) 2->call only test routine
...
#FM_STAR_FEEDBACK_TIME_LIMITER         #turns on time step limiter for stellar evolution
...
#FM_PHOTOELECTRIC_HEATING                  #approx extra molecular cooling contribution addition based on fit to GRACKLE cooling curves
...
#FM_DISCRETE_SN                        #SN feedback is done in discrete SN rather then as a continuous injection
#FM_AGB_WINDS                          #returns momentum and energy to the ISM accounting OB and AGB stellar winds
