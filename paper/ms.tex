...
\edit1{In this work, we have shown that a brief ($\sim300\,\Myr$) period of halted star formation in a narrow \FeH{} bin is capable of producing a bimodal distribution in \alphaFe{} at that \FeH{}. This proposal requires that the \alphaFe{} of gas within the galaxy is decreasing with time so that the gap in star formation translates to a gap in \alphaFe{}. A global quiescent period can satisfy these constraints, but is not necessary. We demonstrate the plausibility of this scenario using a grid of idealized merger simulations with slightly varied orbital parameters -- however, it is also not necessary that a merger triggers the \FeH{}-dependent halt in star formation.}
